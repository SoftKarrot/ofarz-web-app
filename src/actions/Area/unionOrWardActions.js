...
const listUnions = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get("/api/unions");
...
const saveUnion = (union) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post('/api/unions', union)
...
const updateUnion = (union, unionId) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.put('/api/unions/' + unionId, union)
...
const detailsUnion = (unionId) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.get("/api/unions/" + unionId)
...
const deleteUnion = (unionId) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.delete("/api/unions/" + unionId)
